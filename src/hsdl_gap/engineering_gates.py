...
def build_engineering_gate_status(
    ...
    symbolic = payloads["symbolic_explicit_routes"]
    priority = payloads["priority_engine"]
    independent = payloads["independent_javascript_oracle"]

    completeness = {
        "all_candidate_predicates_executable": (
            candidate_ir.get("compilation_mode_counts", {}).get(
                "REQUIRED_FACTS_READINESS_ONLY", 0
            )
            == 0
        ),
        "shared_eu_vn_classification_relation": False,
    ...
    p1_capabilities_ready = all(p1_capabilities.values())
    fully_complete = p0_complete and p1_capabilities_ready and all(
        completeness.values()
    )
    if fully_complete:
        status = "ENGINEERING_COMPLETE"
    elif p0_complete and p1_capabilities_ready:
    ...
    remaining = [
        gate_id for gate_id, complete in completeness.items() if not complete
    ]
    return {
        ...
        "p1_capabilities": {
            "gate_count": len(p1_capabilities),
            "passed": sum(p1_capabilities.values()),
            "ready": p1_capabilities_ready,
            "gates": p1_capabilities,
        },
        "engineering_completeness": completeness,
        "remaining_engineering_work": remaining,
        "summary": {
            "context_count": payloads["decision33_context_v2"].get("context_count"),
            "candidate_rule_count": candidate_ir.get("compiled_rule_count"),
            "candidate_duty_count": candidate_ir.get("compiled_duty_count"),
            "candidate_readiness_only_rule_count": candidate_ir.get(
                "compilation_mode_counts", {}
            ).get("REQUIRED_FACTS_READINESS_ONLY"),
        ...
        "boundary": {
            ...
            "notice": (
                ...
                "completion only. It neither requires nor substitutes for legal review."
            ),
        },
    }
